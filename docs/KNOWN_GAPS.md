...
| Event bus | No production subscriber runs asynchronously | `AsynchronousEventBus` is built, tested and documented, but every wiring in this codebase uses `SynchronousEventBus`, because a deterministic demo and a golden-master test want their subscribers finished before the next line runs. Listed in the README's dead-weight table for the same reason `RiskLimit.and()` is: real machinery with tests, waiting for the live simulation that will use it. |
| Event bus | Unbounded queue, no back-pressure | `AsynchronousEventBus` queues without limit, because bounding it means blocking the publisher - the thing an async bus exists not to do - or dropping events, which needs a stated policy. Nothing in Mercury produces faster than the dispatcher consumes, so a bound would be sized against a guess. A real deployment needs the bound and the policy together. |
