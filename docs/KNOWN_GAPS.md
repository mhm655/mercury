...
| Monte Carlo | Single-threaded | M12 is deliberately single-threaded, per the roadmap - parallel Monte Carlo (`SplittableRandom` per-task seed splitting, scaling benchmarks 1→12 workers) is M13's job. `SplittableRandom` was already chosen in M12 so that work costs no rework, but nothing here calls `.split()` yet. |
